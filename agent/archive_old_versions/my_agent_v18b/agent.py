"""
AlphaSearch Agent V18 — The Limitless AI
...
class Agent:
                    ...
                    for ex, ey in enemies:
                        ...
                        if dist_e <= 5:
                            ...
                            if nmob == 0:
                                ...
                                esc_trap, esc_trap_score = _temporal_escape_scored((nx, ny), hazard, grid, temp_bset, eset, max_d=12, start_depth=2)
                                if esc_trap is not None and esc_trap_score >= -200:
                                    trap_move = a
                                    break
                                ...
                                for a2 in (1, 2, 3, 4):
                                    ...
                                    if _ok(grid, nnx, nny) and (nnx, nny) not in bset and pot_hazard.get((nnx, nny), INF) > 3:
                                        ...
                                        if nmob2 == 0:
                                            ...
                                            esc2_trap, esc2_trap_score = _temporal_escape_scored((nnx, nny), hazard, grid, temp_bset2, eset, max_d=10, start_depth=3)
                                            if esc2_trap is not None and esc2_trap_score >= -200:
                                                trap_move = a
                                                break
        ...
        for x in range(H):
            for y in range(W):
                ...
                if c == ITEM_C or c == ITEM_R:
                    ...
                    # Tactical bonus: item on approach path to enemy
                    if enemies and d_enemy <= 4:
                        score += 20
    ...
    def _should_bomb(self, grid, mpos, radius, bombs, pot_bombs, bset,
        ...
        # ─── LAYER 2: Temporal escape check (Using scored version for quality) ───
        # Try escaping with potential enemy threats modeled
        esc_action, esc_score = _temporal_escape_scored(mpos, new_pot_hazard, grid, new_bset, eset, max_d=14, start_depth=1)
        if esc_action is None or esc_score < -200:
            # Fallback to escaping regular bombs only
            esc_action, esc_score = _temporal_escape_scored(mpos, new_hazard, grid, new_bset, eset, max_d=14, start_depth=1)
            if esc_action is None or esc_score < -200:
                return False
