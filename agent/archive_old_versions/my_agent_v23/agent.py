"""
AlphaSearch Agent V20 — The Infinity AI
...
def _analyze_topology(grid, bset):
    H, W = grid.shape
    deg = {}
    graph = {}
    for x in range(H):
        for y in range(W):
            if 0 <= x < H and 0 <= y < W and grid[x, y] in PASSABLE and (x, y) not in bset:
                nbrs = [(x+dx, y+dy) for dx, dy in DIRS if 0 <= x+dx < H and 0 <= y+dy < W and grid[x+dx, y+dy] in PASSABLE and (x+dx, y+dy) not in bset]
                graph[(x, y)] = nbrs
                deg[(x, y)] = len(nbrs)
    
    q = deque([u for u, d in deg.items() if d <= 1])
    dead_ends = set()
    while q:
        u = q.popleft()
        dead_ends.add(u)
...
def _route_scored(start, target_scores, grid, hazard, bomb_positions, max_d=25):
    if start in bomb_positions and start in target_scores:
        del target_scores[start]
    sorted_targets = sorted(target_scores.items(), key=lambda x: x[1], reverse=True)[:10]
    best_action = None
...
class Agent:
    ...
    def _decide(self, obs):
        ...
        dead_ends = _analyze_topology(grid, bset)
        ...
        # ═══ 4.1 — TIE-BREAKER FRENZY AWARENESS (V22) ════
        if self.step >= 330:
            center_targets = {}
            cx, cy = H // 2, W // 2
            if grid[cx, cy] in PASSABLE and (cx, cy) not in bset:
                center_targets[(cx, cy)] = 3000
            else:
                for dx, dy in DIRS:
                    nx, ny = cx + dx, cy + dy
                    if _ok(grid, nx, ny) and grid[nx, ny] in PASSABLE and (nx, ny) not in bset:
                        center_targets[(nx, ny)] = 2500
            if center_targets:
                if mpos in center_targets:
                    del center_targets[mpos]
                if center_targets:
                    mv = _route_scored(mpos, center_targets, grid, pot_hazard, bset)
                    if mv is not None:
                        return mv

        # ═══ 4.5 — PREDATION TRAP MODE (V21 The Honored One) ════
        if enemies:
            pred_targets = {}
            for ex, ey in enemies:
                if (ex, ey) in dead_ends:
                    q_e = deque([(ex, ey)])
                    seen_e = {(ex, ey)}
                    choke_points = []
                    while q_e:
                        cx, cy = q_e.popleft()
                        if (cx, cy) not in dead_ends and (cx, cy) not in bset and _ok(grid, cx, cy):
                            choke_points.append((cx, cy))
                            break
                        for dx, dy in DIRS:
                            nx, ny = cx + dx, cy + dy
                            if _ok(grid, nx, ny) and (nx, ny) not in seen_e and (nx, ny) not in bset:
                                seen_e.add((nx, ny))
                                q_e.append((nx, ny))
                    for cp in choke_points:
                        if abs(mx - cp[0]) + abs(my - cp[1]) <= 10:
                            pred_targets[cp] = 2000

            if pred_targets:
                if mpos in pred_targets and bombs_left > 0 and mpos not in bset:
                    return 5
                mv = _route_scored(mpos, pred_targets, grid, pot_hazard, bset)
                if mv is not None:
                    return mv
        ...
        for x in range(H):
            for y in range(W):
                ...
                if c == ITEM_C or c == ITEM_R:
                    # V20 Topological Awareness
                    d_enemy = min(abs(x - ex) + abs(y - ey) for ex, ey in enemies) if enemies else 1000000
                    
                    if (x, y) in dead_ends and d_enemy <= 8 and mpos not in dead_ends:
                        continue
                    ...
                    # Tactical bonus: item on approach path to enemy, but ONLY if not in a dead end
                    if enemies and d_enemy <= 4 and exits >= 2 and (x, y) not in dead_ends:
                        score += 20
