"""
AlphaSearch Agent V6 — Competition-Ready Bomberland AI
GDGoC-HCMUS AI Challenge 2026

Built on V5 + breakthrough enhancements:
  1. Removed over-conservative safe_count check: relies purely on temporal escape path.
  2. Active Trap Simulation: simulates enemy escape paths to verify traps.
  3. Chase-Defense Bombing: drops blocking bombs behind when chased.
  4. Chain Farming: checks bombing before escaping (allows safe multiple bomb placement).
  5. Lowered early-game bomb threshold for aggressive clearing.
"""
import numpy as np
...
class Agent:
    ...
    def _decide(self, obs):
        ...
        # ═══ 2 — PLACE BOMB (Section swapped & safety relaxed) ══
        if bombs_left > 0 and mpos not in bset:
            if self._should_bomb(grid, mpos, radius, bombs, bset,
                                 enemies, eset, dm, hazard, blk, players):
                return 5

        # ═══ 3 — TEMPORAL ESCAPE (Non-immediate danger) ═════
        if dm[mx, my] < SAFE:
            esc = _temporal_escape(mpos, hazard, grid, bset, eset)
            if esc is not None:
                return esc
        ...
        # ═══ 4 — COLLECT ITEMS (value-scored routing) ═══════
        item_targets = {}
        for x in range(H):
            for y in range(W):
                c = grid[x, y]
        ...
        # ═══ 5 — MOVE TO BEST BOX SPOTS (scored) ════════════
        farm_targets = {}
    ...
    def _should_bomb(self, grid, mpos, radius, bombs, bset,
                     enemies, eset, dm, hazard, blk, players):
        mx, my = mpos
        ...
        # ─── LAYER 3: safe_count check REMOVED for V6 ───────
        # (Allows aggressive farming in narrow and start-game tiles)
