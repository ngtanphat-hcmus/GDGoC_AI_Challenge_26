"""
AlphaSearch Agent V12 — Flawless Bomberland AI
...
def _temporal_escape(start, hazard, grid, bomb_positions, enemy_positions, max_d=14, start_depth=0):
    ...
    for pos, action, d in safe_options:
        ...
        # 1. DEPTH PENALTY: Heavily penalize long escapes
        score -= d * 50
        ...
        # 4. Anti-Trap: Avoid running towards enemies
        if enemy_positions:
            # First, compute dist from destination to enemies (prefer further)
            min_edist = min(abs(pos[0]-ex) + abs(pos[1]-ey) for ex, ey in enemy_positions)
            score += min_edist * 5
            
            # Second, compute dist from start's NEXT step to enemies
            if action != 0:
                ax, ay = ACT_D[action]
                nx, ny = start[0] + ax, start[1] + ay
                min_start_edist = min(abs(nx-ex) + abs(ny-ey) for ex, ey in enemy_positions)
                
                # If the first step moves us extremely close to an enemy, penalize it!
                if min_start_edist <= 3:
                    score -= 100
