"""
AlphaSearch Agent V12 — The Ultimate AI
...
def _route_scored(start, target_scores, grid, hazard, bomb_positions, enemy_positions, max_d=40):
    ...
    for tpos, tscore in sorted_targets[:10]:
        ...
        if action is not None:
            ...
            H, W = grid.shape
            exits = sum(1 for dx, dy in ((-1,0),(1,0),(0,-1),(0,1)) if 0 <= tpos[0]+dx < H and 0 <= tpos[1]+dy < W and grid[tpos[0]+dx, tpos[1]+dy] in (0,3,4) and (tpos[0]+dx, tpos[1]+dy) not in bomb_positions)
            if exits < 2 and enemy_positions:
                min_edist = min(abs(tpos[0]-ex) + abs(tpos[1]-ey) for ex, ey in enemy_positions)
                if min_edist <= 12:
                    score -= 500
...
class Agent:
    ...
    def _decide(self, obs):
        ...
        if item_targets:
            mv = _route_scored(mpos, item_targets, grid, pot_hazard, bset, enemies)
            if mv is not None:
                return mv
        ...
        if farm_targets:
            ...
            mv = _route_scored(mpos, farm_targets, grid, pot_hazard, bset, enemies)
            if mv is not None:
                return mv
        ...
        if enemies:
            ...
            mv = _route_scored(mpos, pressure_targets, grid, pot_hazard, bset, enemies)
            if mv is not None:
                return mv
