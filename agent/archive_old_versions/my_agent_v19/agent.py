"""
AlphaSearch Agent V19 — The Apex AI
...
class Agent:
    ...
    def _decide(self, obs):
        ...
        # ─── MULTI-THREAT MINIMAX (V19) ────────
        # If ANY enemy is close, assume they place a bomb and add it to potential hazards
        ...
        for i, p in enumerate(players):
            if i != self.agent_id and p[2] == 1:
                ...
                if d <= 5:
                    ...
                    erand = max(1, int(p[4]) + 1)
                    # Add potential enemy bomb with timer=7
                    pot_bombs.append((ex, ey, 7, i, erand))
        ...
        # ─── MINIMAX THREAT MODELING (Multi-Threat V19) ────────
        # If any enemies are close, assume they all place bombs and add to potential hazards
        ...
        for x in range(1, H - 1):
            for y in range(1, W - 1):
                ...
                # V19 Smart Looter Protocol for Farming
                exits = sum(1 for dx, dy in DIRS if _ok(grid, x+dx, y+dy) and (x+dx, y+dy) not in bset)
                if exits < 2 and d_enemy <= 6:
                    continue
