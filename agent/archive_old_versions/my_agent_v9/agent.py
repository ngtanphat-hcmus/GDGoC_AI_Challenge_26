"""
AlphaSearch Agent V9 — Competition-Ready Bomberland AI
GDGoC-HCMUS AI Challenge 2026

Built on V8 + Grandmaster Engine:
  1. Active Herding: reduces enemy mobility to choke them out.
  2. Item Denial: blows up items if an enemy is about to get them.
  3. Unbreakable Survival: deep max_d=20 for chain reactions.
  4. Frenzy Mode: extreme box/item farming at step > 450 for tie-breakers.
"""
import numpy as np
...
class Agent:
        ...
        for x in range(H):
            for y in range(W):
                ...
                if c == ITEM_C or c == ITEM_R:
                    ...
                    if d_enemy < d_me:
                        # Item Denial: If enemy is very close and we can safely bomb it, add to farm targets
                        if d_me <= radius + 2 and d_enemy <= 4:
                            farm_targets[(x, y)] = 250
                        continue  # Enemy is closer, ignore item for collection
    ...
    def _should_bomb(self, grid, mpos, radius, bombs, pot_bombs, bset,
        ...
        esc_action = _temporal_escape(mpos, new_pot_hazard, grid, new_bset, eset, max_d=20, start_depth=1)
        if esc_action is None:
            # Fallback to escaping regular bombs only
            esc_action = _temporal_escape(mpos, new_hazard, grid, new_bset, eset, max_d=20, start_depth=1)
            if esc_action is None:
                return False
