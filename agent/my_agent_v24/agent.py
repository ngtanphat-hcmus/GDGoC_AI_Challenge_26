    ...
    def _decide(self, obs):
                ...
                if (ex, ey) in dead_ends:
                    ...
                    for cp in choke_points:
                        if abs(mx - cp[0]) + abs(my - cp[1]) <= 10:
                            pred_targets[cp] = 4000 if self.step > 350 else 2000
        ...
        for x in range(1, H - 1):
            for y in range(1, W - 1):
                ...
                score = bc * (250 if self.step > 350 else 120)
        ...
        # ═══ 6.5 — TIE-BREAKER BOMB SPAM (Lách luật đặt bom rảnh rỗi) ═══
        if self.step >= 420 and bombs_left > 0:
            bv = self._bomb_value(grid, mpos, radius, bombs, bset, enemies, eset, dm, hazard, blk, players)
            if bv > -500:
                return 5
